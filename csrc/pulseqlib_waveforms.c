...
void pulseqlib_tr_gradient_waveforms_free(pulseqlib_tr_gradient_waveforms *w)
{
    if (!w)
        return;
    if (w->gx.time_us)
        PULSEQLIB_FREE(w->gx.time_us);
    if (w->gx.amplitude_hz_per_m)
        PULSEQLIB_FREE(w->gx.amplitude_hz_per_m);
    if (w->gx.seg_label)
        PULSEQLIB_FREE(w->gx.seg_label);
    if (w->gy.time_us)
        PULSEQLIB_FREE(w->gy.time_us);
    if (w->gy.amplitude_hz_per_m)
        PULSEQLIB_FREE(w->gy.amplitude_hz_per_m);
    if (w->gy.seg_label)
        PULSEQLIB_FREE(w->gy.seg_label);
    if (w->gz.time_us)
        PULSEQLIB_FREE(w->gz.time_us);
    if (w->gz.amplitude_hz_per_m)
        PULSEQLIB_FREE(w->gz.amplitude_hz_per_m);
    if (w->gz.seg_label)
        PULSEQLIB_FREE(w->gz.seg_label);
    memset(w, 0, sizeof(*w));
}
    ...
    int **out_tr_group_labels)
{
    ...
    *out_tr_group_labels = NULL;
    ...
    for (tr_idx = 0; tr_idx < num_trs; ++tr_idx)
    {
        ...
        for (pos = 0; pos < tr_size; ++pos)
        {
            block_idx = tr->num_prep_blocks + tr->imaging_tr_start + tr_idx * tr_size + pos;
            bte = &desc->block_table[block_idx];
    ...
    float target_raster_us)
{
    ...
    {
        if (t_out)
            PULSEQLIB_FREE(t_out);
        if (w_out)
            PULSEQLIB_FREE(w_out);
        return PULSEQLIB_ERR_ALLOC_FAILED;
    }
    ...
    const int *block_order)
{
        ...
        amplitude_mode == PULSEQLIB_AMP_ZERO_VAR)
    {
        ...
        if (!pos_max_gx || !pos_max_gy || !pos_max_gz)
        {
            ...
            if (pos_max_gy)
                PULSEQLIB_FREE(pos_max_gy);
            if (pos_max_gz)
                PULSEQLIB_FREE(pos_max_gz);
        ...
        !time_gz || !wf_gz)
    {
        ...
        if (wf_gx)
            PULSEQLIB_FREE(wf_gx);
        if (wf_gy)
            PULSEQLIB_FREE(wf_gy);
        if (wf_gz)
            PULSEQLIB_FREE(wf_gz);
        diag->code = PULSEQLIB_ERR_ALLOC_FAILED;
        return diag->code;
    }
    ...
    int *out_nch)
{
    ...
    if (decomp_mag.samples)
        PULSEQLIB_FREE(decomp_mag.samples);
    if (decomp_phase.samples)
        PULSEQLIB_FREE(decomp_phase.samples);
    if (decomp_time.samples)
        PULSEQLIB_FREE(decomp_time.samples);
    return idx - start_idx;
}
    ...
    else
    {
        ...
        if (has_nd_prep || has_nd_cool)
        {
            ...
            if (num_avgs_c > 1)
            {
                ...
                {
                    if (can_group_labels)
                        PULSEQLIB_FREE(can_group_labels);
                    if (can_unique_indices)
                        PULSEQLIB_FREE(can_unique_indices);
                    goto alloc_fail;
                }
                n = 0;
...
void pulseqlib_tr_waveforms_free(pulseqlib_tr_waveforms *w)
{
    ...
    if (w->gx.time_us)
        PULSEQLIB_FREE(w->gx.time_us);
    if (w->gx.amplitude)
        PULSEQLIB_FREE(w->gx.amplitude);
    if (w->gy.time_us)
        PULSEQLIB_FREE(w->gy.time_us);
    if (w->gy.amplitude)
        PULSEQLIB_FREE(w->gy.amplitude);
    if (w->gz.time_us)
        PULSEQLIB_FREE(w->gz.time_us);
    if (w->gz.amplitude)
        PULSEQLIB_FREE(w->gz.amplitude);
    if (w->rf_mag.time_us)
        PULSEQLIB_FREE(w->rf_mag.time_us);
    if (w->rf_mag.amplitude)
        PULSEQLIB_FREE(w->rf_mag.amplitude);
    if (w->rf_phase.time_us)
        PULSEQLIB_FREE(w->rf_phase.time_us);
    if (w->rf_phase.amplitude)
        PULSEQLIB_FREE(w->rf_phase.amplitude);
    if (w->adc_events)
        PULSEQLIB_FREE(w->adc_events);
    if (w->blocks)
        PULSEQLIB_FREE(w->blocks);
    memset(w, 0, sizeof(*w));
}
