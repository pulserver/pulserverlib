...
static void seq_file_reset(pulseqlib__seq_file *seq)
{
    ...
    if (seq->is_rf_library_parsed)
    {
        ...
        if (seq->rf_use_tags)
            PULSEQLIB_FREE(seq->rf_use_tags);
        seq->rf_use_tags = NULL;
    }
    if (seq->is_grad_library_parsed)
        PULSEQLIB_FREE(seq->grad_library);
    if (seq->is_adc_library_parsed)
        PULSEQLIB_FREE(seq->adc_library);
    ...
    for (i = 0; i < max_idx; i++)
    {
        ...
        if (n > 0)
        {
            ...
            if (!shapes[i].samples)
            {
                ...
                {
                    if (shapes[j].samples)
                        PULSEQLIB_FREE(shapes[j].samples);
                }
                PULSEQLIB_FREE(shapes);
                return 1;
            }
                                 ...
                                 int n, pulseqlib__scale scale, int flag)
{
    ...
    if (!f)
        return 1;
    if (scale.size > PULSEQLIB__MAX_SCALE_SIZE)
        return 1;
    if (fseek(f, offset, SEEK_SET) != 0)
        return 1;
    if (!fgets(line, sizeof(line), f))
        ...
        else if (strncmp(p, "extension", 9) == 0 && (*(p + 9) == ' ' || *(p + 9) == '\t'))
        {
            ...
            if (sscanf(p, "extension %31s %d", ext_name, &ext_id) == 2)
            {
                if (strcmp(ext_name, "TRIGGERS") == 0)
                    ext_enum = PULSEQLIB__EXT_TRIGGER;
                else if (strcmp(ext_name, "ROTATIONS") == 0)
                    ext_enum = PULSEQLIB__EXT_ROTATION;
                else if (strcmp(ext_name, "LABELSET") == 0)
                    ext_enum = PULSEQLIB__EXT_LABELSET;
                else if (strcmp(ext_name, "LABELINC") == 0)
                    ext_enum = PULSEQLIB__EXT_LABELINC;
                else if (strcmp(ext_name, "RF_SHIMS") == 0)
                    ext_enum = PULSEQLIB__EXT_RF_SHIM;
                else if (strcmp(ext_name, "DELAYS") == 0)
                    ext_enum = PULSEQLIB__EXT_DELAY;
                switch (ext_enum)
                {
                case PULSEQLIB__EXT_TRIGGER:
                    seq->offsets.triggers = pos - (long)strlen(line);
                    break;
                case PULSEQLIB__EXT_ROTATION:
                ...
                case PULSEQLIB__EXT_LABELINC:
                    ...
                    break;
                case PULSEQLIB__EXT_RF_SHIM:
                    seq->offsets.rfshim = pos - (long)strlen(line);
                    break;
                case PULSEQLIB__EXT_DELAY:
                ...
                fgets(use_line, sizeof(use_line), f))
            {
                while (fgets(use_line, sizeof(use_line), f))
                {
                    ...
                    if (col == 10)
                    {
                        ...
                        if (tag == 'e')
                            use_val = PULSEQLIB_RF_USE_EXCITATION;
                        else if (tag == 'r')
                            use_val = PULSEQLIB_RF_USE_REFOCUSING;
                        else if (tag == 'i')
                            use_val = PULSEQLIB_RF_USE_INVERSION;
                        else if (tag == 's')
                            use_val = PULSEQLIB_RF_USE_SATURATION;
                                ...
                                pulseqlib__rf_shim_entry *target, int target_count)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (n_ch > PULSEQLIB__MAX_RF_SHIM_CHANNELS)
            return 1;
...
void pulseqlib__seq_block_free(pulseqlib__seq_block *block)
{
    ...
    if (block->rf.type > 0)
    {
        if (block->rf.mag_shape.samples)
        {
            PULSEQLIB_FREE(block->rf.mag_shape.samples);
            block->rf.mag_shape.samples = NULL;
        }
        if (block->rf.phase_shape.samples)
        {
            PULSEQLIB_FREE(block->rf.phase_shape.samples);
            block->rf.phase_shape.samples = NULL;
        }
        if (block->rf.time_shape.samples)
        {
            PULSEQLIB_FREE(block->rf.time_shape.samples);
            block->rf.time_shape.samples = NULL;
        }
    }
    if (block->gx.type > 1)
    {
        if (block->gx.wave_shape.samples)
        {
            PULSEQLIB_FREE(block->gx.wave_shape.samples);
            block->gx.wave_shape.samples = NULL;
        }
        if (block->gx.time_shape.samples)
        {
            PULSEQLIB_FREE(block->gx.time_shape.samples);
            block->gx.time_shape.samples = NULL;
        }
    }
    if (block->gy.type > 1)
    {
        if (block->gy.wave_shape.samples)
        {
            PULSEQLIB_FREE(block->gy.wave_shape.samples);
            block->gy.wave_shape.samples = NULL;
        }
        if (block->gy.time_shape.samples)
        {
            PULSEQLIB_FREE(block->gy.time_shape.samples);
            block->gy.time_shape.samples = NULL;
        }
    }
    if (block->gz.type > 1)
    {
        if (block->gz.wave_shape.samples)
        {
            PULSEQLIB_FREE(block->gz.wave_shape.samples);
            block->gz.wave_shape.samples = NULL;
        }
        if (block->gz.time_shape.samples)
        {
            PULSEQLIB_FREE(block->gz.time_shape.samples);
            block->gz.time_shape.samples = NULL;
        }
    }
    if (block->adc.type > 0)
    {
        if (block->adc.phase_modulation_shape.samples)
        {
            PULSEQLIB_FREE(block->adc.phase_modulation_shape.samples);
            block->adc.phase_modulation_shape.samples = NULL;
        }
    }
    if (block->rf_shimming.type > 0)
    {
        if (block->rf_shimming.amplitudes)
        {
            PULSEQLIB_FREE(block->rf_shimming.amplitudes);
            block->rf_shimming.amplitudes = NULL;
        }
        if (block->rf_shimming.phases)
        {
            PULSEQLIB_FREE(block->rf_shimming.phases);
            block->rf_shimming.phases = NULL;
        }
    }
}
                                   ...
                                   const pulseqlib_opts *opts)
{
    ...
    for (i = 0; i < num_seq; ++i)
    {
        ...
        {
            for (j = 0; j < i; ++j)
                seq_file_reset(&coll->sequences[j]);
            PULSEQLIB_FREE(coll->sequences);
            PULSEQLIB_FREE(current_path);
            PULSEQLIB_FREE(coll->base_path);
            coll->sequences = NULL;
...
static void extension_block_free(pulseqlib__extension_block *eb)
{
    if (!eb)
        return;
    if (eb->rf_shimming.amplitudes)
    {
        PULSEQLIB_FREE(eb->rf_shimming.amplitudes);
        eb->rf_shimming.amplitudes = NULL;
    }
    if (eb->rf_shimming.phases)
    {
        PULSEQLIB_FREE(eb->rf_shimming.phases);
        eb->rf_shimming.phases = NULL;
    }
    eb->rf_shimming.type = 0;
    eb->rf_shimming.num_channels = 0;
                                  ...
                                  const pulseqlib__raw_extension *re)
{
    ...
    if (ref < 0)
        return 1;
    if (!seq->rf_shim_library || ref >= seq->rf_shim_library_size)
        return 1;
    entry = &seq->rf_shim_library[ref];
    n = entry->num_channels;
    if (n <= 0 || n > PULSEQLIB__MAX_RF_SHIM_CHANNELS)
        return 1;
    ...
    for (i = 0; i < n; ++i)
    {
        amps[i] = entry->values[2 * i];
        phs[i] = entry->values[2 * i + 1];
    }
                                  ...
                                  const pulseqlib__raw_extension *re)
{
    ...
    if (ref < 0)
        return 1;
    if (!seq->trigger_library || ref >= seq->trigger_library_size)
        return 1;
    eb->trigger.type = 1;
                                     ...
                                     const pulseqlib__raw_extension *re)
{
    ...
    if (ref < 0)
        return 1;
    if (!seq->soft_delay_library || ref >= seq->soft_delay_library_size)
        return 1;
                           ...
                           const pulseqlib__raw_extension *re)
{
    ...
    eb->flag = re->flag;
    if (!parse_rotation_from_raw(seq, eb, re))
    {
        extension_block_free(eb);
        return 0;
    }
    if (!parse_rf_shim_from_raw(seq, eb, re))
    {
        extension_block_free(eb);
        return 0;
    }
    if (!parse_trigger_from_raw(seq, eb, re))
    {
        extension_block_free(eb);
        return 0;
    }
    if (!parse_soft_delay_from_raw(seq, eb, re))
    {
        extension_block_free(eb);
        return 0;
    }
                            ...
                            pulseqlib__seq_block *block)
{
    ...
    if (eb->rf_shimming.type && eb->rf_shimming.num_channels > 0)
    {
        ...
        if (amps && phs)
        {
            for (i = 0; i < n; ++i)
            {
                amps[i] = eb->rf_shimming.amplitudes[i];
                phs[i] = eb->rf_shimming.phases[i];
            }
                            ...
                            float grad_raster_us)
{
    ...
    if (raw_idx < 0 || !seq->grad_library || raw_idx >= seq->grad_library_size)
        return 1;
    ...
    else if ((int)fa[0] == 1)
    {
        ...
        if (idx > 0 && seq->is_shapes_library_parsed && idx <= seq->shapes_library_size)
        {
            if (!pulseqlib__decompress_shape(&shape, &seq->shapes_library[idx - 1], grad_raster_us))
                return 0;
            g->time_shape = shape;
        }
        else
                                   ...
                                   const pulseqlib__raw_extension *re)
{
    ...
    /* Gradients */
    if (!parse_grad_event(seq, &block->gx, raw->gx, grad_raster_us))
        goto fail;
    if (!parse_grad_event(seq, &block->gy, raw->gy, grad_raster_us))
        goto fail;
    if (!parse_grad_event(seq, &block->gz, raw->gz, grad_raster_us))
        goto fail;
                          ...
                          pulseqlib__seq_block *block, int block_index)
{
    ...
    if (!pulseqlib__get_raw_block_content_ids(seq, &raw, block_index, 1))
        return;
    has_ext = (raw.ext_count > 0) && seq->is_extensions_library_parsed && seq->extension_lut;

    if (has_ext)
        pulseqlib__get_raw_extension(seq, &re, &raw);
    else
        raw_extension_init(&re);

    if (!parse_block_without_ext(seq, block, &raw, has_ext ? &re : NULL))
        return;
...
int pulseqlib__verify_signature(const char *file_path)
{
    ...
    {
        fclose(f);
        return PULSEQLIB_ERR_SIGNATURE_MISSING;
    }
