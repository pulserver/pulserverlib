...
ISMRMRD::Acquisition GenericConverter::getAcquisition(int dataIndex,
   ...
   const GERecon::Control::ProcessingControl& control)
{
    ...
    // Get sizes
    const auto processingControl = boost::const_pointer_cast<GEProcessing::Control::ProcessingControl>(control.shared_from_this());
    const auto& sliceTable = processingControl->ValueStrict<GERecon::SliceInfoTable>("SliceTable");
    const auto frame_size = processingControl->Value<int>("AcquiredXRes");
    const auto nPhases = processingControl->Value<int>("AcquiredYRes");
    const auto numSlices = processingControl->Value<int>("NumSlices");
    const auto nEchoes = processingControl->Value<int>("NumEchoes");
    const auto nChannels = processingControl->Value<int>("NumChannels");

    // Get view ID and slice ID
    auto viewID  = GERecon::Acquisition::GetPacketValue(programmableControlPacket.viewNumH,  programmableControlPacket.viewNumL);
    auto sliceID = sliceTable.GeometricSliceNumber(GERecon::Acquisition::GetPacketValue(programmableControlPacket.sliceNumH, programmableControlPacket.sliceNumL));
    ...
    else
    {
        ...
        auto kData = frame->Data();

        // Set size of this data frame to receive raw data
        acq.resize(frame_size, nChannels);
        acq.clearAllFlags();
        ...
        idx.slice = sliceID;
        idx.contrast = programmableControlPacket.echoNum;
        ...
        for (auto p = 0; p < ISMRMRD::ISMRMRD_PHYS_STAMPS; p++) {
        ...
        // Negate Orchestra chopping
        const auto chopY = !control.ValueStrict<bool>("ChopY");
        if(chopY)
        {
         if (idx.kspace_encode_step_1 % 2) {
            kData *= -1.0f;
         }
        }
        ...
        for (int channelID = 0 ; channelID < nChannels ; channelID++)
        {
            ...
            {
                acq.data(i, channelID) = kData(i, channelID);
            }
        }
    }
