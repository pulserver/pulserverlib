...
void GadgetronClientConnector::read_task()
{
    ...
    reader_state_.mark_done(read_exception);
}
...
void GadgetronClientConnector::wait() {
    ...
    std::exception_ptr local_ex = reader_state_.consume_exception();
    if (local_ex) {
        std::rethrow_exception(local_ex);
    }
}

bool GadgetronClientConnector::wait_for(unsigned int timeout_ms) {
    if (!reader_state_.wait_for_done(timeout_ms)) {
        return false;
    }
...
int Gadgetron::gadgetron_ismrmrd_client(const std::string& out_filename,
                                        ...
                                        const std::string& out_fileformat,
                                        const unsigned int compression_precision,
                                        const bool query,
                                        const std::string& info,
                                        const float compression_tolerance,
                                        const bool use_zfp_compression,
                                        const bool verbose)
{
    ...
    try
    {
        ...
        if (!info.empty()) {
            con.send_gadgetron_info_query(info);
        }
        else if (!config_xml_local.empty())
        {
        ...
        if (verbose) {
            double transmission_time_s = 0.0;//timer.stop()/1e6;
...
int main(int argc, char **argv)
{
    ...
    const std::string info = vm.count("info") ? vm["info"].as<std::string>() : "";
    const bool query = vm.count("query") > 0;

    return Gadgetron::gadgetron_ismrmrd_client(host_name,
                                               port,
                                               in_filename,
                                               out_filename,
                                               hdf5_in_group,
