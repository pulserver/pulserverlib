"""
Pulseq utility functions.

This module contains low-level helper functions for Pulseq-based
sequence construction and waveform manipulation.
...
def calc_kspace_readout_params(
    ...
    t_read, _, _ = _compute_readout_duration(
...
def split_waveform(
    ...
    """
    return split_gradient(grad, system)
...
def split_waveform_at(
    ...
    ----------
    event : SimpleNamespace
        Event to be split into two gradient events.
    time_point : float
        Time point at which `event` will be split into two waveforms.
    system : Opts, default=Opts()
        System limits.

    Returns
    -------
    event1, event2 : SimpleNamespace
...
def time_revert_waveform(event: SimpleNamespace) -> SimpleNamespace:
    ...
    ----------
    event : SimpleNamespace
        Input Pulseq event. Can be either RF or Grad.
    ...
    if event.type == 'rf':
        output.signal = np.flip(event.signal, axis=-1)
...
def _as_extended_trapezoid(rf: SimpleNamespace, raster: float) -> SimpleNamespace:
    """Convert Pulseq RF to Extended Trapezoid to enable splitting."""
    dummy_system = deepcopy(DUMMY_OPTS)
    dummy_system.max_grad = np.inf
...
def _from_extended_trapezoid(etrap: SimpleNamespace, raster: float) -> SimpleNamespace:
    ...
    rf = make_arbitrary_rf(
        ...
        phase_offset=etrap.phase_offset,
        use=etrap.use,
